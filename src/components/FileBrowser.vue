            ...
            <template v-else>
              ...
              >
                ...
                <div class="fb-files-item-meta">
                  <div class="fb-files-item-meta-size">
...
const selectToggle = (item) => {
  ...
  if (selectedFiles.value.includes(item.path)) {
    ...
    if (
      ...
      selectedFiles.value.length >= props.selectMax
    ) {
...
const filteredExtensions = computed(() => {
  if (props.filterByExtensions) {
    ...
    return props.filterByCategories.reduce(
      (acc, category) => acc.concat(extensionCategories[category]),
      []
    );
  }
  return [];
...
const setContents = async () => {
  ...
  if (!githubImg.state.requests[fullPath]) {
    githubImg.state.requests[fullPath] = github.getContents(
      props.owner,
      props.repo,
      props.branch,
      path.value,
