<template>
    ...
    <div class="error h-screen">
      <div class="text-center max-w-md">
        ...
        <p class="text-neutral-400 mb-6">
          It may have moved or been deleted. Make sure as well things are
          properly configured and that your connection is working.
        </p>
        <div class="flex gap-x-2 justify-center">
          <router-link class="btn-primary" :to="{ name: 'content' }"
            >Take me out of here</router-link
          >
        </div>
      </div>
    </div>
  ...
  <template v-else-if="status == 'no-file'">
    <div class="error h-screen">
      <div class="text-center max-w-md">
        ...
        <div class="flex gap-x-2 justify-center">
          <button class="btn-primary" @click="createSingleFile(schema.path)">
            Create the file
          </button>
        </div>
      </div>
  ...
  <template v-else>
    ...
    >
      <router-link
        v-if="schema && schema.type && schema.type == 'collection'"
        :to="{
          name: 'content',
          params: { name: name },
          ...(folder != schema.path ? { query: { folder: folder } } : {}),
        }"
      >
        ...
        <Dropdown v-if="sha" :dropdownClass="'!max-w-none w-48'">
          ...
          <template #content>
            <ul>
              <li>
                <a
                  :href="`https://github.com/${props.owner}/${props.repo}/blob/${props.branch}/${props.path}`"
                  target="_blank"
                  class="link"
                >
              ...
              >
                ...
                <li>
                  <button
                    class="link w-full"
                    @click.prevent="renameComponent.openModal()"
                  >
                    Rename
                  </button>
                </li>
                <li>
                  <router-link
                    :to="{ name: 'new', params: { path: currentPath } }"
                    class="link w-full"
                    >Make a copy</router-link
                  >
                </li>
                <li>
                  <button
                    class="link-danger w-full"
                    @click.prevent="deleteComponent.openModal()"
                  >
                    Delete
                  </button>
                </li>
    ...
    <main
      ...
      :class="{ 'max-w-4xl': mode !== 'datagrid' }"
    >
      <h1 v-if="displayTitle" class="font-semibold text-2xl lg:text-4xl mb-8">
        {{ displayTitle }}
      </h1>
      <div
...
const schema = computed(() =>
  props.name ? getSchemaByName(props.config, props.name) : null
);
const extension = computed(
  () => schema.value?.extension ?? /(?:\.([^.]+))?$/.exec(props.path)[1]
);
...
const isModelChanged = computed(
  () =>
    JSON.stringify(sanitizeObject(model.value)) !==
    JSON.stringify(sanitizeObject(initialModel.value))
);
...
const createSingleFile = async (path) => {
  ...
  if (data) {
    notifications.notify(
      `The file (${path}) was successfully created.`,
      "success"
    );
    await setEditor();
    status.value = "";
    ...
    notifications.notify(
      `The file (${path}) couldn't be created. Try reloading the page.`,
      "error",
      { delay: 0 }
    );
  }
...
const setDisplayTitle = () => {
  if (props.title) {
    ...
    // Editing a file
    displayTitle.value = model.value?.title
      ? `Editing "${model.value.title}"`
      : "Editing file";
  } else if (props.path) {
...
const setEditor = async () => {
  ...
  // Retrieve the file
  if (props.path) {
    file.value = await github.getFile(
      props.owner,
      props.repo,
      props.branch,
      props.path
  ...
  if (serializedTypes.includes(mode.value) && schema.value?.fields) {
    ...
    try {
      ...
      contentObject = serialization.parse(content, {
        format: mode.value,
      ...
      notifications.notify(
        `Failed to parse the file at "${props.path}", your settings may be wrong.`,
        "error",
        options
      );
    ...
    const dateField = schema.value.fields.find(
    ...
    if (
      ...
      file.value?.name &&
      !contentObject.date &&
      (!schema.value.filename ||
        schema.value.filename.startsWith("{year}-{month}-{day}"))
    ) {
      const filenameDate = getDateFromFilename(file.value.name);
      if (filenameDate) {
        const dateFormat = dateField.options?.format ?? "YYYY-MM-DD";
        contentObject.date = moment(filenameDate.string, "YYYY-MM-DD").format(
          dateFormat
        );
      }
    }
...
const save = async () => {
  ...
  if (validationErrors.length > 0) {
    notifications.notify(
      "Uh-oh! Some of your fields have issues. Please check for errors.",
      "error"
    );
    return;
  }
  ...
  if (serializedTypes.includes(mode.value) && schema.value?.fields) {
    ...
    content = serialization.stringify(contentObject, {
      format: mode.value,
      delimiters: schema.value?.delimiters,
    });
  } else {
  ...
  try {
    ...
    if (!saveData) {
      notifications.notify(
        `Failed to save the file "${currentPath.value}".`,
        "error"
      );
      status.value = "";
      return;
...
const checkConfig = debounce(async () => {
  ({ validation: checkValidation.value } = await config.parse(
    props.owner,
    props.repo,
    props.branch,
    model.value
