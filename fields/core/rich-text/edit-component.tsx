...
import { MediaDialog, type MediaDialogHandle } from "@/components/media/media-dialog";
...
import { extensionCategories, getFileExtension, joinPathSegments, normalizePath } from "@/lib/utils/file";
...
type EditProps = {
  ...
  registerBeforeSubmitHook?: (key: string, hook: () => void | Promise<void>) => () => void;
...
const parseMarkdownTarget = (target: string): { url: string; rest: string; wrapped: boolean } => {
...
const rewriteMarkdownImagesSync = (
  ...
  const imagePattern = /(!\[[^\]]*]\()([^)\n]*)(\))/g;
  return markdown.replace(imagePattern, (_full, prefix: string, target: string, suffix: string) => {
    const parsed = parseMarkdownTarget(target);
    const nextUrl = transformUrl(parsed.url);
    const nextTarget = formatMarkdownTarget(nextUrl, parsed.rest, parsed.wrapped);
...
const EditComponent = forwardRef((props: EditProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  const mediaConfig = useMemo<MediaSchema | undefined>(() => {
    if (!config?.object?.media?.length || options.media === false) return undefined;
    if (options.media && typeof options.media === "string") {
      return getSchemaByName(config.object, options.media, "media") as MediaSchema | undefined;
    }
  ...
  const allowedExtensions = useMemo(() => {
    ...
    } else if (Array.isArray(options.categories) && options.categories.length > 0) {
      extensions = options.categories.flatMap((category) => extensionCategories[category] || []);
    }

    if (Array.isArray(mediaConfig.extensions) && mediaConfig.extensions.length > 0) {
      extensions = extensions.filter((extension) => mediaConfig.extensions?.includes(extension));
    }
  ...
  const toDisplayImageUrl = useCallback(
    async (url: string) => {
      ...
      const inputPath = swapPrefix(url, mediaConfig.output, mediaConfig.input, true);
      const normalizedInputPath = normalizePath(inputPath);
      const mediaInputRoot = normalizePath(mediaConfig.input);
      if (isAbsolutePath(url) && !normalizedInputPath.startsWith(mediaInputRoot)) {
        return url;
      }
  ...
  const toCanonicalImageUrl = useCallback(
    (url: string) => {
      ...
      const relativePath = url.startsWith("https://raw.githubusercontent.com/")
  ...
  const sourceToEditor = useCallback(
    async (source: string) => {
      ...
      if (format === "html") {
        const withInputPrefix = htmlSwapPrefix(source, mediaConfig.output, mediaConfig.input, true);
  ...
  const editorToSource = useCallback(
    async (editorContent: string) => {
      ...
      if (format === "html") {
        ...
        return htmlSwapPrefix(withRelativeUrls, mediaConfig.input, mediaConfig.output);
      }

      const withRelativeMd = rewriteMarkdownImagesSync(editorContent, toCanonicalImageUrl);
      ...
      const withOutputPrefixInMd = rewriteMarkdownImagesSync(withRelativeHtml, (url) =>
        swapPrefix(url, mediaConfig.input, mediaConfig.output),
      );
      return htmlSwapPrefix(withOutputPrefixInMd, mediaConfig.input, mediaConfig.output);
    },
  ...
  const resolvePendingImageSelection = useCallback((result: { kind: "url"; src: string } | null) => {
  ...
  const handleRequestImage = useCallback(async (_context: unknown) => {
    if (!mediaConfig) return null;
    if (pendingImageSelectionRef.current && !pendingImageSelectionRef.current.settled) {
  ...
  const handleUploadImage = useCallback(
    async (file: File) => {
      ...
      const targetPath = joinPathSegments([rootPath ?? mediaConfig.input, file.name]);
