...
export function SignIn() {
  ...
  const errorCallbackURL = callbackURL === "/" ? "/sign-in" : `/sign-in?redirect=${encodeURIComponent(callbackURL)}`;
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 flex justify-center items-center">
      <div className="sm:max-w-[340px] w-full space-y-6">
        ...
        >
          <Input ref={emailInputRef} type="email" name="email" placeholder="Email" required disabled={isSubmitting}/>
          <Button type="submit" className="w-full" disabled={isSubmitting}>
            Sign in with email
            {submittingMethod === "email" && <Loader className="size-4 animate-spin" />}
          </Button>
        </form>
